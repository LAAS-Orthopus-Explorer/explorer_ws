...
public:
  Teleop()
  {
    nh_ = rclcpp::Node::make_shared("teleop");
    nh_->declare_parameter("scale_angular", 1.0);
    nh_->declare_parameter("scale_linear", 1.0);

    twist_pub_ = nh_->create_publisher<geometry_msgs::msg::TwistStamped>("/diff_drive_base_controller/cmd_vel", 10);
  ...
  int keyLoop()
  {
    ...
    std::thread{std::bind(&Teleop::spin, this)}.detach();
    ...
    //     break;
      ...
      // {
        ...
        twist.header.stamp = nh_->now();
        twist.twist.angular.z = nh_->get_parameter("scale_angular").as_double() * angular;
        twist.twist.linear.x = nh_->get_parameter("scale_linear").as_double() * linear;
